...
def build_h5_dataset(data_path, target_path):
    ...
    for i in range(0, 10):
        ...
        mask = inputs_T1 > 0
        # Normalization
        inputs_T1_norm = (inputs_T1 -
                          inputs_T1[mask].mean()) / inputs_T1[mask].std()
        inputs_T2_norm = (inputs_T2 -
                          inputs_T2[mask].mean()) / inputs_T2[mask].std()
        ...
        with h5py.File(
                os.path.join(target_path, 'train_iseg_nocut_%s.h5' % (i + 1)),
                'w') as f:
