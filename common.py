...
note_S = 'Seg_3ddenseseg(Adam lr_S: ' + str(
    lr_S) + ',w_decay:1e-4' + 'beta:' + str(beta1) + ',' + str(
        beta2) + ',' + 'step:' + str(step_size_S) + ' , lr_step)'
note_D = 'Seg_3ddenseseg(Adam lr_S: ' + str(
    lr_S) + ',w_decay:1e-4' + 'beta:' + str(beta1) + ',' + str(
        beta2) + ',' + 'step:' + str(step_size_S) + ' , lr_step)'
...
checkpoint = './checkpoints/' + \
    str(num_checkpoint) + '_' + checkpoint_name + '.pth'
...
def make_one_hot(labels, num_classes):
    ...
    one_hot = torch.cuda.FloatTensor(labels_extend.size(0), num_classes,
                                     labels_extend.size(2),
                                     labels_extend.size(3),
                                     labels_extend.size(4)).zero_()
...
def one_hot(labels):
    ...
    one_hot = np.zeros((labels.shape[0], num_classes, labels.shape[1],
    ...
    for i in range(kernel_size):
        for j in range(kernel_size):
            for k in range(kernel_size):
                weight[0, 0, i, j, k] = (1 - math.fabs(i / f - c)) * (
                    1 - math.fabs(j / f - c)) * (1 - math.fabs(k / f - c))
